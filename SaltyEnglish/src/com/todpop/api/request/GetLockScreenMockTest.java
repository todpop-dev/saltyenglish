 ...
 * 
 */
public class GetLockScreenMockTest extends AsyncTask<Void, Void, JSONObject> {	
    private static GetLockScreenMockTest downloadPronounce = null;
    Context context;
    ...
    public GetLockScreenMockTest(Context context){
     ...
     */
    public static GetLockScreenMockTest getTask(Context context) {
        if(downloadPronounce != null)
        	return downloadPronounce;
        
        return new GetLockScreenMockTest(context);
    }
	...
	protected JSONObject doInBackground(Void... urls) {
		...
		try {
			...
			String getURL = "http://www.todpop.co.kr/api/screen_lock/get_test.json";
			Log.i("JUN", getURL);
				...
				if(json.getBoolean("status") == true){
					...
					for(int i = 0; i < jsonArray.length(); i++){
						...
						if(checkImgHistory(group, id)){	//if image file download history exist, store content in database
							Log.i("junho", "inside checkImgHishtory");
					...
					Log.i("junho", "get lockscreen ad status false");
				}
			}
			else{
				Log.i("junho", "get lockscreen ad return null");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
